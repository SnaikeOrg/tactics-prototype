...
- 2026-09-25 — `/orchestrate` lässt Befunde, die `/pr-review` als „(behebbar)“ markiert, von `/fix` beheben, höchstens 2 Runden, danach an Stephan. Test-Befunde werden über einen neu aufgebauten Test-Commit auf einem neuen Branch behoben, nicht per Force-Push. Mechanische Fehler brauchen keine menschliche Entscheidung, und die Session lässt keinen Force-Push zu.
- 2026-09-25 — Simulation (`packages/sim`): Die Spielerfiguren werden von derselben KI gesteuert wie die Gegner (Profil Aggressive, §23). Der Headless-Kampf braucht eine Steuerung für beide Teams, manuelle Steuerung gibt es in `sim` nicht.
- 2026-09-25 — Simulation (`packages/sim`): Ein Rundenlimit als Parameter beendet den Kampf; wird es erreicht, ohne dass §24 Sieg oder Niederlage meldet, lautet das Ergebnis `ABBRUCH`. Verhindert Endlosschleifen, etwa wenn alle Einheiten nur noch `WAIT` wählen; der Wert ist ein Simulationsparameter, keine Balancing-Zahl.
- 2026-09-26 — `/pr-review`, `/ticket`: Ein Einstiegspunkt (Skript plus Eintrag in `package.json`), der nur Stubs aufruft und keine eigene Logik enthält, gilt im Test-Commit als Stub. Ohne ihn scheitert ein Konsolen-Test an „Command failed“ statt an „not implemented“ (Befund 1 im Review von PR #56).
- 2026-09-26 — `/pr-review`, `/ticket`: Ein benannter Simulationsparameter in einem Einstiegspunkt von `packages/sim`, der als Parameter übergeben wird und als Annahme in `docs/DECISIONS.md` steht, ist von der Prüfung auf Hartkodierung ausgenommen. Er kann kein Testergebnis vortäuschen, weil die geprüfte Funktion ihn nur als Eingabe bekommt (Befund 3 im Review von PR #56).
- 2026-09-26 — Ersetzt ein Ticket eine Übergangslösung, die ein früheres Issue als solche benannt hat, entfernt sein Test-Commit die Tests dieser Übergangslösung; das Issue nennt diese Tests. Begründung: Tests einer Übergangslösung schreiben Verhalten fest, das ausdrücklich ersetzt werden soll.
